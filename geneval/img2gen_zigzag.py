...
from pipeline.pipeline_stable_diffusion_xl import StableDiffusionXLPipeline
...
def main(opt):
    ...
    if opt.model == "stabilityai/stable-diffusion-xl-base-1.0":
        model = StableDiffusionXLPipeline.from_pretrained(opt.model, torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
        model.enable_xformers_memory_efficient_attention()
        inverse_scheduler = DDIMInverseScheduler.from_pretrained('stabilityai/stable-diffusion-xl-base-1.0',
                                                             subfolder='scheduler') 
        model.scheduler = DDIMScheduler.from_config(model.scheduler.config)
        model.inv_scheduler = inverse_scheduler
    else:
    ...
    for index, metadata in enumerate(metadatas):
        ...
        with torch.no_grad():
            all_samples = list()
            final_samples = list()
            for i in trange(opt.k_samples, desc="generation"):
                ...
                for n in trange((opt.n_samples + batch_size - 1) // batch_size, desc="Sampling"):
                    ...
                    samples = model(
                        ...
                        inv_guidance_scale=opt.inv_guidance_scale,
                        T_max=opt.T_max,
                        lambda_step=opt.lambda_step
                    ).images
